...
function UploadBox({ number, title, description, multiple, accept, onChange }: { number: string; title: string; description: string; multiple: boolean; accept: string; onChange: (files: FileList | null) => void }) {
  return <label className="relative flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#bfc4d3] bg-white px-6 text-center transition hover:border-[#4b41e1] hover:bg-[#f5f4ff]"><span className="absolute left-4 top-4 flex h-7 w-7 items-center justify-center rounded-full bg-[#4b41e1] text-xs font-bold text-white">{number}</span><span className="relative flex h-11 w-11 items-center justify-center rounded-full bg-[#eeebff]"><span className="absolute left-1/2 top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-sm bg-[#1f1f1f] before:absolute before:-left-1 before:top-1/2 before:h-2 before:w-2 before:-translate-y-1/2 before:rounded-full before:bg-[#1f1f1f] before:content-[''] after:absolute after:left-1/2 after:-top-1 after:h-2 after:w-2 after:-translate-x-1/2 after:rounded-full after:bg-[#1f1f1f] after:content-['']"/><span className="absolute -right-0.5 top-0 h-2.5 w-2.5 -rotate-12 rounded-sm bg-[#d8d8dd] before:absolute before:left-1/2 before:top-1/2 before:h-1 before:w-1 before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-full before:bg-[#d8d8dd] before:content-['']"/></span><span className="mt-3 font-semibold text-[#17243a]">{title}</span><span className="mt-1 text-sm text-[#686d77]">{description}</span><input type="file" multiple={multiple} accept={accept} onChange={(event) => onChange(event.target.files)} className="sr-only"/></label>;
}
