...
export function Sidebar({ mobileOpen = false, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
  ...
  return <><div className={`${mobileOpen ? "fixed" : "hidden"} inset-0 z-40 bg-[#111827]/35 lg:hidden`} onClick={onClose}/><aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-[#e6e8ef] bg-white transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}><div className="px-3 pb-7 pt-5"><Logo/></div><nav className="flex-1 space-y-2 px-3">{items.map(([icon, label, href]) => { const active = pathname === href; return <Link key={href} href={href} onClick={onClose} className={`flex items-center gap-4 rounded-lg px-4 py-3 text-sm font-medium transition ${active ? "border-r-2 border-[#4b41e1] bg-[#eff1ff] text-[#4438e6]" : "text-[#26354a] hover:bg-[#f5f6fb] hover:text-[#4b41e1]"}`}><Icon name={icon} className="h-5 w-5"/>{label}</Link>; })}</nav><div className="mx-3 pb-3"><Link href="/dashboard/billing" onClick={onClose} className={`mb-3 flex items-center justify-between rounded-xl border px-3 py-3 transition hover:-translate-y-0.5 hover:shadow-sm ${appUser ? planStyles[appUser.plan] : planStyles.BASIC}`}><span><span className="block text-xs font-semibold uppercase tracking-wide">{planName} plan</span><span className="mt-0.5 block text-[11px] opacity-75">View plan & billing</span></span><span aria-hidden="true" className="text-base">→</span></Link><div className="border-t border-[#e6e8ef] pt-3"><Link href="/dashboard/settings" onClick={onClose} className="flex items-center gap-4 rounded-lg px-4 py-3 text-sm text-[#26354a] hover:bg-[#f5f6fb]"><Icon name="settings" className="h-5 w-5"/>Settings</Link><button onClick={handleLogout} title="Log out" className="mt-2 flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left hover:bg-[#f5f6fb]"><span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#172033] text-xs font-semibold text-white">{appUser?.photoURL ? <img src={appUser.photoURL} alt="" className="h-full w-full object-cover"/> : initial}</span><span className="min-w-0 flex-1 truncate text-xs font-semibold text-[#0b1c30]">{appUser?.name || "User"}</span><Icon name="logout" className="h-4 w-4 text-[#7b8190]"/></button></div></div></aside></>;
}
