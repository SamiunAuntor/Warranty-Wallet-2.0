...
export type Subscription = { id: string; plan: Plan; scheduledPlan: Plan | null; pendingPlan: Plan | null; paymentUrl?: string | null; status: "ACTIVE" | "INCOMPLETE" | "PAST_DUE" | "EXPIRED" | "CANCELLED"; startsAt: string; expiresAt: string; currentPeriodEnd: string | null; cancelAtPeriodEnd: boolean; isActive: boolean } | null;
export type Payment = { id: string; amount: string; currency: string; plan: Plan | null; status: "PENDING" | "SUCCESS" | "FAILED" | "REFUNDED"; createdAt: string };
export type PaymentList = { data: Payment[]; meta: { page: number; limit: number; total: number; totalPages: number } };
