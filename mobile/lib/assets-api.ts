...
export type Asset = {
  ...
  lifecycleStatus: "ADDED" | "ARCHIVED";
...
export function getAsset(token: string, id: string) {
  return apiRequest<Asset>(`/products/${id}`, { token });
}
export function createAsset(token: string, input: AssetInput) {
  return apiRequest<Asset>("/products", {
    method: "POST",
    token,
...
export function updateAsset(token: string, id: string, input: Partial<AssetInput>) {
  return apiRequest<Asset>(`/products/${id}`, {
    method: "PATCH",
    token,
    body: JSON.stringify(input),
  });
...
export function deleteAsset(token: string, id: string) {
  return apiRequest<null>(`/products/${id}`, { method: "DELETE", token });
}
